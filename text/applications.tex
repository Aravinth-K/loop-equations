...
\begin{align}
    ...
    W  p_{2}+ W_{2} = & \frac{\beta  W_{(2,2,2)}}{\alpha  \beta -\gamma ^2}-b\Delta^2 W_{(2,2)}-d\Delta W_{(2,2,2)}-d W_{(2,1,2,2)} -e W_{(2,2,2,2)}+\frac{\gamma \Delta W_{(2,2)}}{\gamma ^2-\alpha  \beta },\\
...
\end{align}
%
These are eight independent equations in eight unknowns which may be eliminated to give $F(z,W(z))=0$, with $(\deg_x F,\deg_y F)=(5,4)$ for generic couplings. We can, at most, tune the parameters of the model to achieve critical behaviour of the form $W(z)\sim(z-z_c)^{4/3}$, which is characteristic of the critical Ising model universality class.


We write each spectral curve as $F(x,y)=0$, with $F_{(p)}(x,y)=0$
for the boundary condition associated with a sum of $p$ matrices. The relation
between $(x,y)$ and the resolvent is stated with each curve. Coefficients
containing undetermined trace correlation functions are denoted by $c_{i,j}$,
where $i$ and $j$ are the exponents of $x$ and $y$. These coefficients depend on
the couplings and the correlators; they are not independent parameters.
The complete expressions are supplied by the accompanying notebooks.
